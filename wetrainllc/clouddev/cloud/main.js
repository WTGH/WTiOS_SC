...
Parse.Cloud.define("startWorkout", function(request, response) {
    ...
    query.get(workoutId, {
        success: function(workout){
            ...
            ///check workout has discount
            var promoCode = workout.get("promoCode")
            
            if (promoCode) {
              
              var disCountPercentage = promoCode.get("discountPercentage")
              
              console.log("disCountPercentage " + disCountPercentage)
              console.log("disCountAmount " + (amt * (disCountPercentage/100))

              amt = amt - (amt * (disCountPercentage/100))
                          
              console.log("workOutAmount " + amt);
...
var createCharge = function(client, payment, response) {
    ...
    ///
    var workout = client.get("workout")
    var promocode = = workout.get("promoCode")
    
    var relation = promocode.relation("UsedClients");
    relation.add(client);
    promocode.save();
                   
    client.set("promoCode", promocode)
    client.save();
