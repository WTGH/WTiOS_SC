...
class RatingView : UIView {
    ...
    func updateRating(isRated : Bool){
        ...
        if  let trainer: PFObject = PFUser.currentUser()!.objectForKey("trainer") as? PFObject {
            trainer.fetchInBackgroundWithBlock({ (trainer, error) -> Void in
        
                if let woRequest: PFObject = trainer!.objectForKey("workout") as? PFObject {
                    woRequest.fetchInBackgroundWithBlock({ (woRequest, error) -> Void in
                          print("request: \(woRequest)")
                        ///get clients's user object
                        if let client: PFObject = woRequest!.objectForKey("client") as? PFObject {
                                ...
                                if let User: PFObject = object!.objectForKey("user") as? PFObject {
                                    User.fetchInBackgroundWithBlock({ (Userobject, error) -> Void in
                                        ...
                                        if error == nil{
                                            ...
                                            request.setObject("client", forKey: "ratingForUserRole")
                                            ...
                                            request.saveInBackgroundWithBlock { (success, error) -> Void in
                                                ...
                                                if success == true
                                                {
                                                    woRequest!.setObject(request, forKey: "trainerRating")
