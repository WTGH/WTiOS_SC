...
class ClientInfoViewController: UIViewController, UITextFieldDelegate, MFMessageComposeViewControllerDelegate {
    ...
    func updateLabelInfo() {
        ...
        if  let _ : PFObject = self.request.objectForKey("scheduleInfo") as? PFObject {
            
            let schTime = self.request.objectForKey("scheduledTime") as? NSDate
            if schTime != nil {
                info = "\(info)Scheduled for: " + (AppDelegate.formatScheduleTime(schTime) as String) + "\n"
            }
        }
        else
        {
            info = "\(info)Scheduled for: " + "Now!\n"
        ...
        if let avgRating: NSNumber = self.client!.objectForKey("avgRating") as? NSNumber {
            
            if avgRating.floatValue > 0{
                info = "\(info)Average Rating: " + String(format:"%.1f", avgRating.floatValue)
            }
        }
        
        let pt: CGFloat = 15
        let attrs: [String: AnyObject] = [NSFontAttributeName : UIFont(name: "HelveticaNeue", size: pt)!] as [String: AnyObject]
        let attributedInfo: NSMutableAttributedString = NSMutableAttributedString(string: info, attributes: attrs)
        self.labelInfo.attributedText = attributedInfo
        
        if notes != nil {
            // Parse notes
            let notesTitle: NSMutableAttributedString = NSMutableAttributedString(string: "\nClient notes:\n\n")
            let attrs: [String: AnyObject] = [NSFontAttributeName : UIFont(name: "HelveticaNeue-Bold", size: pt)!] as [String: AnyObject]
                ...
                if request.objectId != nil {
                    ...
                    if request.objectForKey("trainerRating") == nil{
                        ...
                        request.fetchIfNeededInBackgroundWithBlock { (object, error) -> Void in
                            ...
                            if self.request.objectForKey("trainerRating") == nil{
                                self.showRatingView()
                            }
                        }
    ...
    func validatePasscode() {
        ...
        if let promocode: PFObject = self.request!.objectForKey("promoCode") as? PFObject {
            
            promocode.fetchInBackgroundWithBlock({ (object, error) -> Void in
                
                print("promocode:")
                print(promocode["promoCode"] as! String?)

                var validCode = promocode["promoCode"] as! String?
                
                if validCode?.characters.count == 0{
                    validCode = self.request.objectForKey("passcode") as? String
                }
    ...
    */
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        // Get the new view controller using segue.destinationViewController.
        // Pass the selected object to the new view controller.
        if segue.identifier == "GoToUploadStatus" {
            let request = sender as! PFObject
            let controller = segue.destinationViewController as! VideoUploadStatus
