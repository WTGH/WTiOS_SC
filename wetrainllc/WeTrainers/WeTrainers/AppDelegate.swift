...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didRegisterUserNotificationSettings notificationSettings: UIUserNotificationSettings) {
        ...
        if self.window!.rootViewController!.isKindOfClass(UINavigationController) {
            
            let cNavController = self.window!.rootViewController as! UINavigationController
            
            if cNavController.visibleViewController!.isKindOfClass(ConnectViewController){
                
                let cController = cNavController.visibleViewController as! ConnectViewController
                cController.refreshStatus()
    ...
    static func formatScheduleTime (var scheduledDate : NSDate!) -> NSString {
        
        let dateFormatter : NSDateFormatter = NSDateFormatter()
        dateFormatter.dateFormat = "hh:mm a"
        scheduledDate = scheduledDate.dateByAddingTimeInterval( -(15*60) )   //// show 15 min before from sch time
        dateFormatter.timeZone = NSTimeZone.defaultTimeZone()
        dateFormatter.locale   = NSLocale.systemLocale()
        return dateFormatter.stringFromDate(scheduledDate)
    }
