...
class SignupViewController: UIViewController, UITextFieldDelegate, UIGestureRecognizerDelegate, TutorialDelegate {
    ...
    @IBOutlet var buttonAge: UIButton!
    
    @IBOutlet var buttonTOSbg: UIButton!
    @IBOutlet var buttonAgebg: UIButton!
    ...
    @IBAction func didClickCheck(button : UIButton) {
        ...
        if self.buttonTOS == button  || self.buttonTOSbg == button{
    ...
    @IBAction func didClickFacebookSignUp(button : UIButton) {
        
        self.loginWithFacebook(self,isCheckedTos: true)
    }
    
    func getUserBasicInfoFromFacebook(){
        
        if FBSDKAccessToken.currentAccessToken() != nil {
            
            FBSDKGraphRequest(graphPath: "me", parameters: ["fields": "id, name, first_name, last_name, email,age_range"]).startWithCompletionHandler({ (connection, result, error) -> Void in
    ...
    func didSignup(sender: AnyObject) {
        ...
        if self.inputConfirmation.text! != self.inputPassword.text! {
            self.simpleAlert("Please make sure passwords match", message: nil)
            return
        }
