...
class TrainerProfileViewController: UIViewController, MFMessageComposeViewControllerDelegate {
    ...
    @IBOutlet weak var constraintContactTop: NSLayoutConstraint!
    ...
    override func viewDidLoad() {
        ...
        trainer?.fetchInBackgroundWithBlock({ (object, error) -> Void in
            ...
            if status == RequestState.Complete.rawValue {
                self.updateRequestState()
            }
            
        })
        
        if (self.request!.objectForKey("promoCode") != nil) {
            
            if let request: PFObject = self.request!.objectForKey("promoCode") as? PFObject {
    ...
    func updateTrainerInfo() {
        ...
        if status == RequestState.Matched.rawValue {
            ...
            let promocode: PFObject!
            var passcode: String!
            
            ///replace passcode by promocode if available
            if (self.request!.objectForKey("promoCode") != nil) {
                promocode = self.request!.objectForKey("promoCode") as! PFObject
                
                if promocode.allKeys.contains("promoCode") {
                
                    if promocode["promoCode"] != nil {
                        passcode = promocode["promoCode"] as! String?
                    }
                } else {
                    print("promocode not available yet")
                }
            ...
            if passcode == nil {
                passcode = self.request!.objectForKey("passcode") as! String
            } else if passcode.characters.count == 0 {
                passcode = self.request!.objectForKey("passcode") as! String
            }
        ...
        if let avgRating: NSNumber = self.trainer!.objectForKey("avgRating") as? NSNumber {
            
            if avgRating.floatValue > 0{
                self.avgRatinglbl.text = "Average Rating: " + String(format:"%.1f", avgRating.floatValue)
            } else {
                self.avgRatinglbl.text = ""
            }
        ...
        if self.avgRatinglbl.text?.characters.count == 0 {
            self.constraintContactTop.constant =  10
        }
                ...
                if self.timer != nil {
                 ...
                 if status == RequestState.Training.rawValue
                 {
                    if self.request!.objectId != nil {
                        ...
                        if self.request!.objectForKey("clientRating") == nil{
                            self.showRatingView()
                        }
                    }
